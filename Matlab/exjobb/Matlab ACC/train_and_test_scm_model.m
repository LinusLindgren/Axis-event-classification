...
featureVector = [sum_auto(:,3) min_auto(:,2:3) cross_corr_max(2,:)' meanFeatures' ...
 minFeatures' ...
maxFeatures(1:2,:)'  skewness_vec(:,2:3) kurtosis_acor() skewness_acor ...
sum_changes'  squeeze(psdx_peak_freq_bin_tilt([1 4],1,:))' squeeze(psdx_peak_freq_bin_tilt(:,3,:))'...
skewness_psdx psdx_nbrPeaks_tilt' ...
squeeze(psdx_peak_freq_bin([1 3 4 5],1,:))' squeeze(psdx_peak_freq_bin([1 2 4 5],2,:))' index_of_first_max(3,:)' meanTiltFeatures(2,:)'  stdFeatures(2,:)' sum_changes_auto'];
